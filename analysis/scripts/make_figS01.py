...
## Construct tables.
table = table.pivot_table('response',['subscale','item'],'study').reset_index()
table = table.merge(dif.loc[dif.variable == 'study', ['item','coef']])
...
for i, (x, y) in enumerate(zip(table.teicher2015, table.tuominen2022)):
    ax.plot([x,y], [i,i], color='0.5')

## Plot points.
sns.stripplot(x='teicher2015', y='item', data=table, order=order, color='0.2', orient='h', 
              jitter=False, size=8, linewidth=1, edgecolor='w', ax=ax)
sns.stripplot(x='tuominen2022', y='item', data=table, order=order, color='#dd8452', orient='h', 
              jitter=False, size=8, linewidth=1, edgecolor='w', ax=ax)
...
## Define palette.
palette = np.where(table.coef < -0.638, '0.2', np.where(table.coef > 0.638, '#dd8452', '0.8'))
...
## Add legend.
ax.scatter([], [], s=80, color='0.2', label='Original sample')
ax.scatter([], [], s=80, color='#dd8452', label='Replication sample')
ax.set(xticks=[], yticks=[], xlabel='', ylabel='')
ax.legend(loc=10, ncol=2, frameon=False, columnspacing=1.5, handletextpad=0, fontsize=14)

sns.despine(left=True, right=True, top=True, bottom=True)
plt.savefig(os.path.join(ROOT_DIR, '..', 'figures', 'figS01.png'), dpi=100)