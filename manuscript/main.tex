...
\shorttitle{Factorial validity of the MACE}
...
\noindent \textbf{Objective}: The Maltreatment and Abuse Chronology of Exposure (MACE) scale is a retrospective self-report measure of childhood maltreatment. Despite increasing use, the factor structure of the MACE has not been previously investigated. As such, the psycho- metric properties of the MACE total and subscale scores are indeterminate. The aims of the current study was to examine the factorial validity of the MACE and the reliability of its total and subscale scores.

{\begin{spacing}{1.25} \hfill \\ \end{spacing}}

\noindent \textbf{Methods}: We analyzed the response data from two independent samples of participants (N=1051 \& N=582) using confirmatory factor analysis. We fit a series of models with increasing dimensionality to explore factorial validity. Furthermore, we used model-based indices to estimate the reliability of the MACE total and subscale scores, and to test for essential unidimensionality. 

{\begin{spacing}{1.25} \hfill \\ \end{spacing}}

\noindent \textbf{Results}: Confirmatory factor analysis indicated that a bifactor S-1 model with two specific factors (peer victimization, reverse-scoring) provided the best and most parsimonious fit to the data. Despite this, model-based indices suggested the MACE scale is essentially unidimensional. We found the MACE total score was a reliable and valid measure of overall childhood maltreatment. In contrast, the MACE subscale scores exhibited little reliable variance after controlling for general maltreatment.
